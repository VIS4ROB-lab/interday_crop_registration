# Support retrained LoFTR models
Two files has been modified to add configurations and allow downloading model weight files from self-provided links. Two retrained models used in the paper are already embedded in the code.

## How to use customized retained LoFTR models
* [`hloc/match_dense.py`](./hloc/match_dense.py): add the configuration of two retrained loftr models. Follow the template below. 
```shell
'loftr_23_0.5':  # name of the model, specifying the model to use
{
    'output': 'matches-loftr',
    'model': {
        'name': 'loftr',
        'weights': 'loftr_23_0.5',  # name to the download link of the weight file, should correspond to the definition in the `urls` dictionary
    },
    'preprocessing': {
        'grayscale': True,
        'resize_max': 1024,
...
* [`hloc/matchers/loftr.py`](./hloc/matchers/loftr.py): add links in the `urls` dictionary to download the weight file of retrained models. Examples:
```shell
